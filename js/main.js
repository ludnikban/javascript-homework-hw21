...
function  checkEmail(str) {
  return (/^(([a-zA-Zа-яА-ЯёЁ]+\.?\-?[a-zA-Zа-яА-ЯёЁ]+@[a-zA-Z.]+\.[a-z]{2,3}))$/g).test(str);
}
...
function checkDomainUrl(str) {
  return (/^(https?\:\/\/([a-z0-9-_]+\.){1,5}[a-z]+)$/g).test(str);
}
...
function createLinksFromDomains(str) {
  str.match(/https?\:\/\/([a-z0-9-_]+\.){1,5}[a-z]+/g).forEach((entry) => str = str.replace(entry,`<a href="${entry}" target="_blank">${entry.replace(/https?\:\/\//gi, '')}</a>`));
  return str;  
  };
